            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-8 print:hidden">
                    ...
                    <div className="flex justify-between items-start">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Invoice Details</h1>
                ...
                <div className="bg-white dark:bg-[rgb(var(--color-card))] rounded-xl shadow-sm dark:shadow-lg border dark:border-[rgb(var(--color-border))] p-8 print:shadow-none">
                    ...
                    <div className="border-b dark:border-[rgb(var(--color-border))] pb-6 mb-6">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <div className="text-lg font-semibold text-gray-900 dark:text-gray-100">{invoice.invoiceNo}</div>
                            </div>
                            <div className="text-right">
                                <div className="text-sm text-gray-600 mb-1 dark:text-gray-400">Invoice Date</div>
                    ...
                    <div className="grid grid-cols-2 gap-8 mb-8">
                        <div>
                            <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2 dark:text-gray-400">Bill To</h3>
                            {invoice.customer ? (
                                <div>
                                    <div className="font-bold text-gray-900 text-lg dark:text-gray-200">{invoice.customer.name}</div>
                                    <div className="text-gray-600 dark:text-gray-200 mt-1">{invoice.customer.phone}</div>
                                    {invoice.customer.email && (
                                        <div className="text-gray-600 dark:text-gray-200">{invoice.customer.email}</div>
                                    )}
                                    {invoice.customer.address && (
                                        <div className="text-gray-600 dark:text-gray-200 mt-1">{invoice.customer.address}</div>
                                    )}
                                </div>
                            ) : (
                                <div className="text-gray-600 dark:text-gray-400">Walk-in Customer</div>
                            )}
                        </div>

                        <div className="text-right">
                            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400uppercase mb-2">Payment Status</h3>
                            ...
                            <div className="mt-4">
                                ...
                                <div className="font-medium text-gray-900 dark:text-gray-200 capitalize">{paymentMethodDisplay}</div>
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b-2 border-gray-300">
                                    <th className="text-left py-3 px-2 font-semibold text-gray-700 dark:text-gray-200 ">#</th>
                                    <th className="text-left py-3 px-2 font-semibold text-gray-700 dark:text-gray-200 ">Item</th>
                                    <th className="text-right py-3 px-2 font-semibold text-gray-700 dark:text-gray-200 ">Quantity</th>
                                    <th className="text-right py-3 px-2 font-semibold text-gray-700 dark:text-gray-200 ">Price</th>
                                    <th className="text-right py-3 px-2 font-semibold text-gray-700 dark:text-gray-200 ">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                {items.map((item, index) => (
                                    <tr key={index} className="border-b border-gray-200">
                                        <td className="py-3 px-2 text-gray-600 dark:text-gray-200 ">{index + 1}</td>
                                        <td className="py-3 px-2 text-gray-900 dark:text-gray-200 ">{item.name || 'Item'}</td>
                                        <td className="py-3 px-2 text-right text-gray-900 dark:text-gray-300 ">{item.quantity}</td>
                                        <td className="py-3 px-2 text-right text-gray-900 dark:text-gray-300 ">₹{fmt(item.price)}</td>
                                        <td className="py-3 px-2 text-right font-medium text-gray-900 dark:text-gray-300 ">
                                            ₹{fmt(item.total)}
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className="flex justify-end">
                        <div className="w-64">
                            <div className="flex justify-between py-2 border-b">
                                ...
                                <span className="font-medium text-gray-900 dark:text-gray-200 ">₹{fmt(invoice.subtotal)}</span>
                            </div>
                            {invoice.discount > 0 && (
                                <div className="flex justify-between py-2 border-b">
                                    <span className="text-gray-600 dark:text-gray-300">Discount:</span>
                                    <span className="font-medium text-red-600 dark:text-gray-200">-₹{fmt(invoice.discount)}</span>
                                </div>
                            )}
                            {invoice.previousDueAmount > 0 && (
                                <div className="flex justify-between py-2 border-b">
                                    <span className="text-gray-600 dark:text-gray-300">Previous Due Added:</span>
                                    <span className="font-medium text-amber-600 dark:text-gray-200 ">+₹{fmt(invoice.previousDueAmount)}</span>
                                </div>
                            )}
                            <div className="flex justify-between py-3 border-b-2 border-gray-300">
                                <span className="text-lg font-bold text-gray-900 dark:text-gray-300">Total Amount:</span>
                                <span className="text-lg font-bold text-indigo-600 dark:text-gray-200">
                                    ₹{fmt(invoice.totalAmount)}
                                </span>
                            </div>
                            {invoice.creditApplied > 0 && (
                                <div className="flex justify-between py-2 border-b">
                                    <span className="text-gray-600 dark:text-gray-300">Credit Applied:</span>
                                    <span className="font-medium text-green-600 dark:text-gray-200">-₹{fmt(invoice.creditApplied)}</span>
                                </div>
                            )}
                            <div className="flex justify-between py-2 border-b">
                                ...
                                <span className="font-medium text-green-600 dark:text-gray-200">₹{fmt(invoice.paidAmount)}</span>
                            </div>
                            {invoice.creditApplied > 0 && (
                                <div className="mt-2 pt-3 border-t border-gray-200 space-y-2">
                                    <div className="text-sm font-semibold text-gray-600 dark:text-gray-300">Payment Breakdown</div>
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600 dark:text-gray-300">Customer Credit:</span>
                                        <span className="font-medium text-gray-900 dark:text-gray-200">₹{fmt(invoice.creditApplied)}</span>
                                    </div>
                                    {invoice.paidAmount > 0 && (
                                        <>
                                            {invoice.splitPaymentDetails && invoice.splitPaymentDetails.length > 0 ? (
                                                invoice.splitPaymentDetails.map((split, idx) => (
                                                    <div key={idx} className="flex justify-between text-sm">
                                                        <span className="text-gray-600 dark:text-gray-300">{formatPaymentMethodLabel(split.method)}:</span>
                                                        <span className="font-medium text-gray-900 dark:text-gray-200">₹{fmt(split.amount)}</span>
                                                    </div>
                                                ))
                                            ) : (
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-gray-600 dark:text-gray-300">{formatPaymentMethodLabel(invoice.paidViaMethod || invoice.paymentMethod)}:</span>
                                                    <span className="font-medium text-gray-900 dark:text-gray-200">₹{fmt(invoice.paidAmount)}</span>
                                                </div>
                                            )}
                                        </>
                                    )}
                                ...
                                if (balanceDue > 0) {
                                    return (
                                        <div className="flex justify-between py-2">
                                            <span className="text-gray-600 font-medium dark:text-gray-300">Balance Due:</span>
                                            <span className="font-bold text-red-600 dark:text-gray-200">
                                    ...
                                    return (
                                        <div className="flex justify-between py-2">
                                            <span className="text-gray-600 font-medium dark:text-gray-300">Paid in Full</span>
                                            <span className="font-bold text-green-600 dark:text-gray-200">✓</span>
                                        </div>
                                    );
                                }
                            })()}
